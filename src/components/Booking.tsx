"use client";

import { useState } from "react";
import ScrollReveal from "./ScrollReveal";

export default function Booking() {
  const [open, setOpen] = useState(false);

  return (
    <section id="booking" className="relative py-24 lg:py-32">
      {/* Background accent */}
      <div className="absolute inset-0 bg-gradient-to-b from-charcoal via-charcoal-light/50 to-charcoal" />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <ScrollReveal direction="up" delay={0}>
        <div className="text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-gold">
            Schedule a Visit
          </p>
          <h2 className="mt-4 font-heading text-4xl font-bold text-cream sm:text-5xl">
            Book Your <span className="text-gold-gradient">Appointment</span>
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-silver-light/60">
            Walk-ins welcome. Or book online in under a minute and
            guarantee your spot.
          </p>
        </div>
        </ScrollReveal>

        {/* CTA + alternatives */}
        <ScrollReveal direction="up" delay={200} duration={800}>
        <div className="mt-12 mx-auto max-w-5xl">
          <div className="flex justify-center mb-8">
            <button
              onClick={() => setOpen(true)}
              className="rounded-sm bg-gold px-10 py-4 text-base font-semibold uppercase tracking-wider text-charcoal transition-all hover:bg-gold-light hover:shadow-xl hover:shadow-gold/20"
            >
              Book Your Appointment
            </button>
          </div>

          {/* Alternative booking options */}
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <a
              href="tel:+353870366366"
              className="inline-flex items-center gap-2 rounded-sm border border-gold/30 px-6 py-3 text-sm font-medium text-gold transition-all hover:border-gold hover:bg-gold/10"
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
              </svg>
              Call to Book: 087 036 6366
            </a>
            <a
              href="https://wa.me/353870366366"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-sm border border-green-600/30 px-6 py-3 text-sm font-medium text-green-400 transition-all hover:border-green-600 hover:bg-green-600/10"
            >
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              WhatsApp Us
            </a>
          </div>
        </div>
        </ScrollReveal>
      </div>

      {/* Booking Modal */}
      {open && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/90 backdrop-blur-sm p-4"
          onClick={() => setOpen(false)}
        >
          <div
            className="relative w-full max-w-3xl rounded-sm border border-white/10 bg-charcoal shadow-2xl shadow-black/60 overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal header */}
            <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
              <p className="font-heading text-lg font-semibold text-cream">
                Book an Appointment
              </p>
              <button
                onClick={() => setOpen(false)}
                className="text-cream/50 transition-colors hover:text-gold"
                aria-label="Close"
              >
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
            {/* Setmore iframe */}
            <iframe
              src="https://faithfulbarbers.setmore.com"
              title="Book an appointment at Faithful Barbers"
              width="100%"
              height="600"
              style={{ border: 0 }}
              className="block w-full overflow-auto"
            />
          </div>
        </div>
      )}
    </section>
  );
}
